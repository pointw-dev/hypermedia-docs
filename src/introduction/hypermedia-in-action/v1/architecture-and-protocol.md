...
|  3   | Client sent a GET request for all venues, calling the static method `Venue.select_venue()`<br/>**alice**:<br/>&nbsp;&nbsp;`url = Api.url_from_resource(root, 'venue')->GET`<br/>**bob**:<br/>&nbsp;&nbsp;`url = Api.url_join(BASE_API_URL, '/venues')->GET`                                                                 |
...
|  6   | Client sends a GET request for all events that belong to that venue by calling `venue.get_events()`.<br/>**alice**:<br/>&nbsp;&nbsp;`url = Api.url_from_resource(venue, 'event')->GET`<br/>**bob**:<br/>&nbsp;&nbsp;`url = Api.url_join(BASE_API_URL, f'/venues/{venuef["_id"]}/events')->GET`                              |
...
|  9   | Client called `event.view_attendees()`<br/>**alice**:<br/>&nbsp;&nbsp;`url = Api.url_from_resource(event, 'registration')->GET`<br/>**bob**:<br/>&nbsp;&nbsp;`url = Api.url_join(BASE_API_URL, f'/events/{event["_id"]}/registrations')->GET`                                                                               |
...
|  12  | Client tells the venue to send a POST request with its body containing the event details: `venue.add_event(event)`.<br/>**alice**:<br/>&nbsp;&nbsp;`url = Api.url_from_resource(venue, 'event')->POST event`<br/>**bob**:<br/>&nbsp;&nbsp;`url = Api.url_join(BASE_API_URL, f'/venues/{venue["_id"]}/events')->POST event`  |
